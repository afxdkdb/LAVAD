...
st.sidebar.title("配置")
st.sidebar.text_input("云端服务器地址", value="http://8.160.176.96:8000", key="cloud_url_input")
cloud_url = st.session_state.get("cloud_url_input", "http://8.160.176.96:8000")
st.sidebar.markdown("---")
st.sidebar.info("""
**使用说明：**
1. 点击下方按钮上传视频文件
...
with col2:
    ...
    if st.session_state["result"]:
        ...
        if anomalous_frames:
            ...
            for i, frame_info in enumerate(anomaly_details):
                ...
                with st.expander(f"⚠️ 帧 {frame_num} | 时间: {time_sec}s | 类型: {', '.join(anomaly_types)} | 置信度: {score}"):
                    ...
                    with col_img:
                        try:
                            ...
                            st.image(frame_url, caption=f"帧 {frame_num}", width=200)
                        except Exception as e:
                            st.warning(f"无法加载帧图片: {str(e)[:50]}")
                            st.info(f"图片URL: {frame_url}")
                    with col_info:
